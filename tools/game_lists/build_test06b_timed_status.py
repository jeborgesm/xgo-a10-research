#!/usr/bin/env python3
"""Build XGO Game-List Test06b: explicit staged Refresh action with timed status.
...
Test06b changes only the User Menu UI/navigation/dispatch surface:
...
def build_dispatch_writer():
    ...
    a.ins(jal(OS_GET_TICK)); a.ins(nop()); a.loadlabel('t0','status_tick'); a.ins(sw('v0',0,'t0'))
    ...
    a.ins(jal(OS_GET_TICK)); a.ins(nop())
    a.loadlabel('t1','status_tick'); a.ins(lw('t1',0,'t1'))
    a.ins(subu('t2','v0','t1')); a.ins(sltiu('t1','t2',STATUS_MS))
    a.branch('bne','t1','zero','status_live'); a.ins(nop())
    a.loadlabel('t1','status'); a.ins(sw('zero',0,'t1'))
    a.branch('beq','zero','zero','status_done'); a.ins(nop())
    a.label('status_live')
    a.loadlabel('t0','status'); a.ins(lw('t0',0,'t0'))
    ...
    a.label('status_tick'); a.data(struct.pack('<I',0))
