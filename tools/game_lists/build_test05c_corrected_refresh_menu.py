#!/usr/bin/env python3
"""Build XGO Game-List Test05c: corrected explicit Refresh menu (no writes).
...
Test05c changes only the User Menu UI/navigation/dispatch surface:
...
def draw_ascii(dst,text,cx,y,scale=3):
    # Deterministic bold raster: first render the 5x7 glyphs, then apply a
    # one-pixel dilation in all four cardinal directions. This keeps the
    # title-case label compact but gives it weight closer to the OEM labels.
                ...
                if on=='1':
                    for sy in range(scale):
                        for sx in range(scale):
                            pts.add((x+xx*scale+sx,y+yy*scale+sy))
        x += 6*scale
    bold=set(pts)
    for px,py in pts:
        bold.update(((px-1,py),(px+1,py),(px,py-1),(px,py+1)))
    for px,py in bold:
        if 0<=px<640 and 0<=py<480: pix_set(dst,px,py)
...
def build_screen(src,blank,name):
    ...
    draw_ascii(dst,'Refresh',441,398,3)
    return bytes(dst)
