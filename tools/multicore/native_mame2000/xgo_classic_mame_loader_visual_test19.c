/*
 * XGO Classic Arcade MAME2000 Test19 visual loader trace.
 *
 * No filesystem writes. The current stock Loading screen is annotated using
 * the proven OEM text renderer at three pre-entry checkpoints:
 * C = immediately before stock sound-task shutdown
 * D = immediately after stock sound-task shutdown returns
...
static void (*const text_draw)(void *,int,int,int,u32,u32,const char *) = (void *)0x803528a4;
...
static void visual_stage(const char *msg)
{
    u32 gp,s5;
    void *font;
    u32 cfg;
    __asm__ volatile("move %0,$28":"=r"(gp));
    __asm__ volatile("move %0,$21":"=r"(s5));
    font=*(void **)(gp-5136u);
    cfg=*(volatile u32 *)(gp-30380u);
    text_draw(font,300,210,0,s5,cfg,msg);
    dly_tsk(300);
}
