...
int __core_entry_c(const char *filename,int load_state)
{
    ...
    if(i==0 || i>=128 || j+5>=sizeof(zip_path))
        return -1;
    zip_path[j++]='/'; zip_path[j++]='b'; zip_path[j++]='i';
    zip_path[j++]='n'; zip_path[j++]='/';
    ...
    /* MAME2000 0.37b5 exposes libretro serialize stubs returning size 0.
     * Keep callbacks installed so stock UI failure is graceful rather than
     * falling into stale embedded-FBA handlers. */
