/* Native-main-list Snes9x2005 frontend for XGO Core #2. */
#include <stddef.h>
typedef int bool;
#define true 1
#define false 0
#define XGO_SYSTEM_SNES 0x0008u
...
int __core_entry_c(const char *filename,int load_state)
{
    ...
    if(!ROM_BUFFER||!rom_size||rom_size>MAX_ROM_SIZE)
        return -1;
    ...
    /* This is intentional: stock run_emulator() family 0x08 selects the
     * hardware-confirmed 11025-Hz SNES audio profile and skips generic AV
     * sound initialization. */
    SYSTEM_FAMILY=XGO_SYSTEM_SNES;
    ...
    GFN_GET_AV=xgo_core_get_av; /* installed for completeness; 0x08 skips it */
