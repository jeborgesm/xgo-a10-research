/*
 * Native SNES external-core loader for XGO.
 *
 * Intended patch site:
 *   run_game + SNES dispatch JAL at 0x80360e40
 *
 * run_game() has already preloaded the selected SNES ROM into gp_buf_64m before
 ...
 * untouched stock run_snes() implementation.
 */
 ...
 * launch the same ROM through untouched stock run_snes(), giving the hardware an
...
static void (*const stock_run_snes)(const char *, int) = (void *)0x8035f9d8;
...
void load_and_run_core(const char *filename, int load_state)
{
    ...
    /* If upper RAM is already in active stock use, do not disturb SNES at all. */
    if (*HEAP_BREAK >= CORE_BASE) {
        stock_run_snes(filename, load_state);
        return;
    }

    /*
     * Validate the tiny XGOC header while stock memory/task state is untouched.
     * XGO's own run_snes() begins by stopping the sound task; do the same before
     * changing RAMSIZE or writing the large external image into upper RAM.
     */
    f = fw_fopen("/mnt/sda1/cores/snes9x2005/core.xgc", "rb");
    if (!f)
    ...
    /* Match the first action of the real XGO run_snes() before memory takeover. */
    ...
    *RAMSIZE = old_limit;
     ...
     * Launching stock SNES then gives a user-visible PASS without invoking any
