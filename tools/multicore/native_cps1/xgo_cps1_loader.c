 ...
 * launch the same ROM through untouched stock run_fba(), giving the hardware an
...
void load_and_run_core(const char *filename, int load_state)
{
    ...
    /* Match the first action of the real XGO run_fba() before memory takeover. */
    ...
    *RAMSIZE = old_limit;
     ...
     * Launching stock CPS1 then gives a user-visible PASS without invoking any
