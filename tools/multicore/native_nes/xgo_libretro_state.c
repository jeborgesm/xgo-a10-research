 ...
 *
 * Hardware Test 01 reached the stock save UI but reported:
 *   "Archive save failed. Please check TF card"
 *
 * Test 02 therefore records the callback pathname and last successful stage to
 * /mnt/sda1/xgo-state-probe.txt using the already-proven low-level stock fs_*
 * veneers. The diagnostic is intentionally independent of newlib stdio.
 */
...
static void probe_stage(const char *stage, const char *path)
{
    ...
    unsigned n = 0;
    int fd;
    n = append_text(buf, n, sizeof(buf), "stage=");
    n = append_text(buf, n, sizeof(buf), stage);
    n = append_text(buf, n, sizeof(buf), "\npath=");
    n = append_text(buf, n, sizeof(buf), path ? path : "(null)");
    n = append_text(buf, n, sizeof(buf), "\n");
    fd = xgo_stock_fs_open(PROBE_PATH, FS_O_WRONLY | FS_O_CREAT | FS_O_TRUNC, 0666);
    if (fd >= 0) {
        (void)xgo_stock_fs_write(fd, buf, n);
...
static int suffix_is(const char *path, const char *suffix)
{
    ...
    for (p = path; *p; ++p) ++path_len;
    for (s = suffix; *s; ++s) ++suffix_len;
...
{
    return suffix_is(path, ".sa0") || suffix_is(path, ".sa1") ||
           suffix_is(path, ".sa2") || suffix_is(path, ".sa3") ||
           suffix_is(path, ".skp");
}
...
{
    FILE *file = NULL;
    void *raw = NULL, *compressed = NULL;
    size_t raw_size, compressed_capacity;
    unsigned long compressed_len;
    uint32_t stored_len;
    int ok = 0;

    probe_stage("S1-save-route", path);
    if (!path || !*path) return 0;

    raw_size = retro_serialize_size();
    if (raw_size == 0 || raw_size > XGO_STATE_MAX_SERIALIZED) {
        probe_stage("S2-serialize-size-fail", path); return 0;
    }
    probe_stage("S2-serialize-size-ok", path);

    if (raw_size > (XGO_STATE_MAX_COMPRESSED / 2u)) return 0;
    compressed_capacity = raw_size * 2u;
    if (!compressed_capacity || compressed_capacity > XGO_STATE_MAX_COMPRESSED) return 0;

    raw = malloc(raw_size);
    if (!raw) { probe_stage("S3-raw-malloc-fail", path); goto out; }
    ...
    if (!compressed) { probe_stage("S5-compressed-malloc-fail", path); goto out; }
    probe_stage("S5-compressed-malloc-ok", path);

    compressed_len = (unsigned long)compressed_capacity;
    if (xgo_stock_state_compress(compressed, &compressed_len, raw, (unsigned long)raw_size) != 0) {
        probe_stage("S6-compress-fail", path); goto out;
    }
    if (!compressed_len || compressed_len > compressed_capacity || compressed_len > 0xffffffffu) {
        probe_stage("S6-compress-length-fail", path); goto out;
    }
    probe_stage("S6-compress-ok", path);
    stored_len = (uint32_t)compressed_len;

    file = fopen(path, "wb");
    if (!file) { probe_stage("S7-fopen-fail", path); goto out; }
    ...
    probe_stage("S10-save-success", path);
    ok = 1;
out:
    if (file) fclose(file);
...
static int xgo_state_load_prefix(const char *path)
{
    FILE *file = NULL;
    void *raw = NULL, *compressed = NULL;
    size_t serialize_size, raw_capacity;
    unsigned long raw_len;
    uint32_t stored_len = 0;
    int ok = 0;

    probe_stage("L1-load-route", path);
    if (!path || !*path) return 0;
    file = fopen(path, "rb");
    if (!file) { probe_stage("L2-fopen-fail", path); goto out; }
    if (fread(&stored_len, sizeof(stored_len), 1, file) != 1) goto out;
    if (!stored_len || stored_len > XGO_STATE_MAX_COMPRESSED) goto out;
    compressed = malloc((size_t)stored_len);
    if (!compressed) goto out;
    if (fread(compressed, 1, stored_len, file) != stored_len) goto out;
    serialize_size = retro_serialize_size();
    if (!serialize_size || serialize_size > XGO_STATE_MAX_SERIALIZED) goto out;
    if (serialize_size > (XGO_STATE_MAX_SERIALIZED / 2u)) goto out;
    raw_capacity = serialize_size * 2u;
    raw = malloc(raw_capacity);
    if (!raw) goto out;
    raw_len = (unsigned long)raw_capacity;
    if (xgo_stock_state_uncompress(raw, &raw_len, compressed, (unsigned long)stored_len) != 0) goto out;
    if (!raw_len || raw_len > raw_capacity) goto out;
    if (!retro_unserialize(raw, (size_t)raw_len)) goto out;
    probe_stage("L9-load-success", path);
    ok = 1;
out:
    if (file) fclose(file);
    if (compressed) free(compressed);
