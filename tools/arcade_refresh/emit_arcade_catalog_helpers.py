...
def patch_pair(b,ho,lo,target):
    oldh=rd32(b,ho); oldl=rd32(b,lo); rt=(oldh>>16)&31
    assert oldh>>26==0x0f
...
def emit(parent:bytes,fam,out:pathlib.Path):
    ...
    for key in ("slot0","slot1","slot2","root"):
        offsets[key]=len(b); b.extend(strings[key].encode("ascii")+b"\0")
    # Keep the parent's original rb/wb literals. No code references appended
    # mode strings, so duplicating them would only enlarge the command-6 load.
    for ho,lo,key in REFS: patch_pair(b,ho,lo,runtime(offsets[key]))
